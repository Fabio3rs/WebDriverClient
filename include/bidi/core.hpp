...
class BiDiSession : public std::enable_shared_from_this<BiDiSession> {
  ...
  public:
    ...
    /**
     * @brief TEST ONLY: Clear all pending responses
     *
     * @warning Must be called from strand context for thread safety.
     *          Forcefully discards all in-flight requests without waiting
     *          for responses. Use only during cleanup/shutdown.
     */
    void test_pending_clear() noexcept {
        try {
    ...
    /**
     * @brief TEST ONLY: Clear all event handlers
     *
     * @warning Must be called from strand context for thread safety.
     *          Removes all locally registered event handlers without
     *          unsubscribing from the server.
     */
    void test_clear_event_handlers() noexcept {
        try {
            event_handlers_.clear();
            event_refcount_.clear();
            context_event_refcount_.clear();
        } catch (...) {
            // Silently ignore - we're cleaning up anyway
        }
    }
